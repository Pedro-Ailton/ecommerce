...
class Categoria_Produto(db.Model):
    ...
    produto = db.relationship('Produtos', backref='categoria_produto')
    categoria = db.relationship('Categorias', backref='categoria_produto')
...
class Pedidos(db.Model):
    ...
    produto = db.relationship('Produtos', backref='pedidos')
    cliente = db.relationship('Clientes', backref='pedidos')
...
class Movimentacoes(db.Model):
    ...
    data_mov = db.Column(db.DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), nullable=False)
    ...
    def data_mov_br(self):
        if self.data_mov.tzinfo is None:
            dt = self.data_mov.replace(tzinfo=timezone.utc)
        else:
            dt = self.data_mov
