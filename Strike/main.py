...
db.init_app(app)
pasta_banners = 'static/Img/BANNERS/'

# =================== Página inicial ===================

@app.route('/')
def index():
    lista_banners = [f for f in os.listdir(pasta_banners) if os.path.isfile(os.path.join(pasta_banners, f))]
    if 'usuario' in session and session.get('tipo') == 'Cliente':
        ...
        return render_template('index.html', usuario=session['usuario'], produtos=produtos, nome_usuario=nome_usuario, imagens=imagens, banners = lista_banners)
    else:
        produtos = db.session.execute(db.select(Produtos)).scalars().all()
        imagens = db.session.execute(db.select(ImagemProduto)).scalars().all()
        return render_template('index.html',produtos=produtos,banners = lista_banners, imagens=imagens)
...
@app.route('/admin/deletar', methods=['GET','POST'])
def deletar_item():
    ...
    if 'usuario' in session and session.get('tipo') == 'Administrador':
        ...
        if request.method == "POST":
            id = int(request.form['produto_id'])
        
            produto = Produtos.query.get(id)
            categoria_produto = Categoria_Produto.query.filter_by(produto_id = id).first()
            estoque = Estoque.query.filter_by(produto_id = id).first()
            movimentacao = Movimentacoes.query.filter_by(produto_id = id).first()
            if movimentacao:
...
def finalizar_compra():
    if 'usuario' not in session:
        flash('Você precisa estar logado para adicionar endereço.')
        return redirect(url_for('login'))
...
def produto(id):
    produto = Produtos.query.get(id)
    fotos = ImagemProduto.query.filter_by(produto_id=id).all()

    return render_template('produto.html', produto=produto, fotos=fotos)

# ================== Rota de Gerenciar categorias ==================
""""essa rota tem que:
- adicionar categorias
- mostrar as categorias já criadas
- mostrar os produtos associados a cada categoria
- permitir associar um produto a uma categoria ou mais
- permitir desassociar um produto de uma categoria
- permitir deletar categorias (apenas se não tiver produto associado)
"""
@app.route('/admin/categorias', methods=['POST', 'GET'])
def admin_categoria():
    if 'usuario' in session and session.get('tipo') == 'Administrador':
        produtos = Produtos.query.all()
        categoria_produtos = Categoria_Produto.query.all()
        categorias = Categorias.query.all()
        if request.method == 'POST':
            if 'add_categoria' in request.form:
                nome_categoria = request.form['nome_categoria']
                categoria_existente = Categorias.query.filter_by(nome = nome_categoria).first()
                if not nome_categoria:
                    return redirect('/admin/categorias')
                elif categoria_existente:
                    return redirect('/admin/categorias')
                else:
                    nova_categoria = Categorias(nome = nome_categoria)
                    db.session.add(nova_categoria)
                    db.session.commit()
                    return redirect('/admin/categorias')
            elif 'prod_cat' in request.form:
                produto_id = request.form['produto_id']
                categoria_id = request.form['categoria_id']
                if not produto_id or not categoria_id:
                    flash('Produto ou categoria não selecionados!')
                    return redirect('/admin/categorias')
                else:
                    prod_cat_existente = Categoria_Produto.query.filter_by(produto_id = produto_id, categoria_id = categoria_id).first()
                    if prod_cat_existente:
                        flash("Esse produto já está associado a essa categoria!")
                        return redirect('/admin/categorias')
                    else:
                        novo_prod_cat = Categoria_Produto(produto_id = produto_id, categoria_id = categoria_id)
                        db.session.add(novo_prod_cat)
                        db.session.commit()
                        flash("Produto associado à categoria com sucesso!")
                        return redirect("/admin/categorias")
            elif 'deassasociar' in request.form:
                produto_id = request.form('produto_id')
                categoria_id = request.form('categoria_id')
                ...
                if not produto_id or not categoria_id:
                    ...
                    prod_cat_existente = Categoria_Produto.query.filter_by(produto_id = produto_id, categoria_id= categoria_id).first()
                    if not prod_cat_existente:
                        flash(f'A associação entre o produto {produto_id} e da categoria {categoria_id} não foi encontrada')
                        return redirect('/admin/categorias')
                    else:
                        db.session.delete(prod_cat_existente)
                        db.session.commit()
                        flash('Associação deletada com sucesso!')
                        return redirect('/admin/categorias')
            elif 'deletar_categoria' in request.form:
                categoria_id = request.form('categoria_id')
                if not categoria_id:
                    flash('A categoria não foi preenchida!')
                    return redirect('/admin/categorias')
                else:
                    categoria_existente = Categorias.query.filter_by(categoria_id = categoria_id).first()
                    if categoria_existente:
                        associacoes = Categoria_Produto.query.filter_by(categoria_id=categoria_id).all()
        ...
        return render_template('gerenciar_categorias.html', produtos = produtos, categoria_produtos = categoria_produtos, categorias = categorias)

# =================== Rota de gerenciar banners ===================
